...
async function getTeamsFromAirtable() {
  if (Object.keys(teams).length) {
    return teams;
  } // return teams is already cached
...
async function getSlackMembersForATeam(channel) {
  ...
  for await (const page of slack.paginate('conversations.members', {
    channel,
  })) {
    const { members } = page;
...
async function getChannelMemberships() {
  ...
  const airtableTeams = await getTeamsFromAirtable();
  console.log(teams);
  const slackTeamIds = Object.keys(airtableTeams);
  await Promise.all(
    slackTeamIds.map(
      throat(1, async (slackTeamId) => {
        const { id, name } = airtableTeams[slackTeamId];
...
async function getMembershipMap(slackUsers, channelMembership) {
  ...
  slackUsers.forEach((slackUser) => {
    ...
    Object.values(airtableTeams).forEach(({ id: teamAirtableId }) => {
